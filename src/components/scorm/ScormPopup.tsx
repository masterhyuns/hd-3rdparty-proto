...
const ScormPopup: React.FC = () => {
	...
	const callLMSInitialize = () => {
		if (popupRef.current) {
			...
			console.error("Popup is not opened!");
		}
	};
	
	const callLMSGetValue = () => {
		if (popupRef.current) {
			popupRef.current.postMessage(
				{
					type: "scormApiCall",
					method: "LMSGetValue",
					params: ["cmi.core.score.raw"],
				},
				"https://cloud.scorm.com"
			);
		} else {
	...
	return (
		<div>
			<h1>SCORM Player (Popup)</h1>
			<button onClick={openPopup}>Open SCORM Player</button>
			<button onClick={callLMSInitialize}>Call LMSInitialize</button>
			<button onClick={callLMSGetValue}>Call LMSGetValue</button>
		</div>
	);
};

export default ScormPopup;
